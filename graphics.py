...
_font_cache = {}
ImageSource = str | os.PathLike[str]
...
def _get_root() -> tk.Tk:
    ...
    if _root is None:
        try:
            ...
            raise RuntimeError(
                "graphics.py requires a usable Tk installation, but this Python runtime "
                f"could not initialize Tcl/Tk: {exc}"
            ) from exc
    return _root
...
def _scale_photo_image(image: tk.PhotoImage, scale: int) -> tk.PhotoImage:
    if scale > 1:
        return image.zoom(scale)
    return image


def _create_photo_image(*pixmap: object, scale: int = 1) -> tk.PhotoImage:
    root = _get_root()
    if len(pixmap) == 1:
        image_source = cast(ImageSource, pixmap[0])
        image_path = os.fspath(image_source)
        try:
            image = tk.PhotoImage(file=image_path, master=root)
        except tk.TclError:
            image_bytes = Path(image_path).read_bytes()
            encoded = base64.b64encode(image_bytes).decode("ascii")
            if image_bytes.startswith(b"\x89PNG\r\n\x1a\n"):
                image = tk.PhotoImage(data=encoded, format="png", master=root)
            else:
                image = tk.PhotoImage(data=encoded, master=root)
        return _scale_photo_image(image, scale)
    if len(pixmap) != 2:
        raise GraphicsError(BAD_OPTION)
    width = int(cast(int, pixmap[0]))
    height = int(cast(int, pixmap[1]))
    image = tk.PhotoImage(master=root, width=width, height=height)
    return _scale_photo_image(image, scale)


def _measure_text(text: str, size: int = 12, style: str = "normal", face: str = "helvetica") -> tuple[int, int]:
    root = _get_root()
    weight = "bold" if "bold" in style else "normal"
    slant = "italic" if "italic" in style else "roman"
    key = (face, size, weight, slant)
    font = _font_cache.get(key)
    if font is None:
        font = tkfont.Font(root=root, family=face, size=size, weight=weight, slant=slant)
        _font_cache[key] = font

    width = font.measure(text or " ")
    height = font.metrics("linespace")
    return int(width), int(height)


def measure_text(text: str, size: int = 12, style: str = "normal", face: str = "helvetica") -> tuple[int, int]:
    return _measure_text(text, size=size, style=style, face=face)
...
class GraphWin(tk.Canvas):
    ...
    def __init__(self, title="Graphics Window",
        ...
        self.items = []
        self.mouseX: Optional[float] = None
        self.mouseY: Optional[float] = None
        self.bind("<Button-1>", self._onClick)
        self.height = height
        self.width = width
        self.autoflush = autoflush
        self._mouseCallback: Optional[Callable[[Point], None]] = None
...
class GraphicsObject:
    ...
    def _draw(self, canvas, options) -> int:
        """draws appropriate figure on canvas with options provided
        Returns Tk id of item drawn"""
        raise NotImplementedError("GraphicsObject subclasses must implement _draw")
...
class Entry(GraphicsObject):
    ...
    def __init__(self, p, width):
        ...
        #print self.anchor
        self.width = width
        self.text = tk.StringVar(_get_root())
        self.text.set("")
        self.fill = "gray"
        self.color = "black"
...
class Image(GraphicsObject):
    ...
    def __init__(self, p, *pixmap, scale: int = 1):
        GraphicsObject.__init__(self, [])
        self.anchor = p.clone()
        self.imageId = Image.idCount
        Image.idCount = Image.idCount + 1
        self.img = _create_photo_image(*pixmap, scale=scale)
...
def create_scaled_image(anchor: Point, image_path: ImageSource, scale: int = 1) -> Image:
    return Image(anchor, image_path, scale=scale)
